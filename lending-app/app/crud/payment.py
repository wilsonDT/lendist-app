...
async def get_payment(db: Session, payment_id: int) -> Optional[Payment]:
    result = await db.exec(select(Payment).where(Payment.id == payment_id))
    return result.first()

async def get_payments(db: Session, skip: int = 0, limit: int = 100) -> List[Payment]:
    result = await db.exec(select(Payment).offset(skip).limit(limit))
    return result.all()

async def get_payments_by_loan(db: Session, loan_id: int) -> List[Payment]:
    result = await db.exec(select(Payment).where(Payment.loan_id == loan_id))
    return result.all()
